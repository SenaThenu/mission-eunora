...
def generate_terrain(floor, front, block_side):
    n_prepare = WIDTH // 96
    if front:
        del floor[0:n_prepare]
        last_x = floor[-1].rect.x
        for i in range(1, n_prepare+1):
            floor.append(
                Block(last_x+(block_side*i), WIDTH - block_side, block_side))
    else:
        del floor[-n_prepare:-1]
        last_x = floor[-1].rect.x
        for i in range(1, n_prepare+1):
            floor.append(Block(last_x+(block_side*i),
                         WIDTH - block_side, block_side))


def scroll_bg(player, floor):
    global OFFSET_X
    if player.rect.x > WIDTH - 190:
        OFFSET_X += PLAYER_VEL
...
def main():
    ...
    player = Player(500, 0, 32, 32, "Ninja Frog")
